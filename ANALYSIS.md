...
| Memobase | ✅ (typed 8-topic / ~40-slot user-profile ontology, per-project customizable) | ⚠️ (BufferZone staging before flush) | ⚠️ (GeneralBlob ephemeral post-flush unless configured persistent) | ✅ (UserEvent timeline + UserEventGist with pgvector) | ✅ (UserProfile rows with topic::sub_topic schema) | ❌ | ❌ | ❌ | ⚠️ (YOLO merge APPEND/UPDATE/ABORT via prompt; no code-level dedup/decay) | ✅ (reproducible LoCoMo artifacts: 75.78 overall / 85.05 temporal shipped in `docs/experiments/locomo-benchmark/`) |
| Cognee | ⚠️ (implicit via session Q&A cognified into graph) | ✅ (session cache: Redis/FS, `{user_id}:{session_id}`) | ⚠️ (Data records + raw files, per-dataset scope) | ⚠️ (session Q&A entries; no typed Episode primitive) | ✅ (graph nodes via LLM extraction, DataPoint schema) | ❌ | ⚠️ (dataset as project-level scope) | ✅ (multi-backend: Kuzu/Neo4j/Neptune; triplet embeddings) | ✅ (`improve()` feedback loop: apply feedback weights + cognify sessions + triplet embeddings + sync graph→session) | ⚠️ (HotpotQA vs Mem0/Graphiti/LightRAG; no LoCoMo/LongMemEval) |
...
- 2026-04-25: Added **Memobase** (memodb-io) and **Cognee** (topoteretes) to §2 comparison matrix after full code-level analyses. Memobase: user-profile-ontology system with reproducible LoCoMo artifacts (75.78 overall / 85.05 temporal, beats Zep and Mem0). Cognee: knowledge-graph + session-cache engine with bidirectional session-graph sync and feedback-weighted consolidation (16.8k stars, Apache-2.0). Standalone analyses: `ANALYSIS-memobase.md`, `ANALYSIS-cognee.md`. Second Me (mindverse) was examined but **not promoted** — it's a personal LLM fine-tuning pipeline, not a runtime memory system; kept as analyzed-but-excluded reference in `ANALYSIS-second-me.md`.
