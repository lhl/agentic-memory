...
|------|--------|--------|---------|
| 2026-04-25 | Memobase (memodb-io) | [memodb-io/memobase](https://github.com/memodb-io/memobase) | **PROMOTED to standalone analysis + ANALYSIS.md matrix.** User-profile-based memory system (FastAPI + Postgres + Redis + pgvector; Apache-2.0; 2.7k stars). Typed user-profile ontology (8 topics / ~40 slots, customizable), cold-path batch architecture (buffer → fixed 3-call YOLO merge), dual profile/event channels with pgvector gists. Reproducible LoCoMo artifacts shipped: overall 75.78, temporal 85.05 (best-in-table, beats Zep 67.71, Mem0 55.51). Verified against source code at commit `358c16bb`. Gaps: no provenance/versioning/trust labels, LoCoMo-only evaluation, "sub-100ms" latency claim is architectural (not benchmarked). See `ANALYSIS-memobase.md`. |
| 2026-04-25 | Cognee (topoteretes) | [topoteretes/cognee](https://github.com/topoteretes/cognee) — arXiv:[2505.24478](https://arxiv.org/abs/2505.24478) | **PROMOTED to standalone analysis + ANALYSIS.md matrix.** Knowledge-graph-backed memory engine (~138k LOC Python, Apache-2.0, 16.8k stars, v1.0.3 at commit `f4964c31`). Multi-backend (Kuzu/Neo4j/Neptune + LanceDB/Chroma/pgvector/Qdrant/Weaviate/Milvus + SQLite/Postgres). Central novelty: **bidirectional session-graph sync with feedback-weighted consolidation** via `improve()` (apply feedback weights → cognify sessions into graph → triplet embeddings → sync edges back to session cache). 13+ search types with rule-based router, `@agent_memory` decorator, multi-tenant RBAC with shared datasets. Caveats: no LoCoMo/LongMemEval (only HotpotQA), no supersedes/bi-temporal, hard-delete-only `forget()`, per-chunk extraction (cost-unpredictable). Paper contributes one mechanism within a larger platform. See `ANALYSIS-cognee.md`. |
| 2026-04-25 | Second Me (mindverse) | [mindverse/Second-Me](https://github.com/mindverse/Second-Me) — arXiv:[2503.08102](https://arxiv.org/abs/2503.08102) | **Not promoted — scope-mismatched.** Initially triaged as a promotion candidate, but code reading reveals it's a **personal LLM fine-tuning pipeline**, not a runtime memory system. "Memory" lives in LoRA adapter weights (~13.8k LOC across L0/L1/L2: document insights → biography synthesis → synthetic QA → LoRA SFT + optional DPO → GGUF → llama.cpp). Update latency 30–60 min (full re-training). "Hierarchical Memory Modeling" is the training-data pipeline; "Me-Alignment Algorithm" is standard LoRA + DPO (TRL's SFTTrainer/DPOTrainer, r=64 α=16). No agent loop, no tool use, no LoCoMo/LongMemEval. Only L0 has runtime retrieval (ChromaDB for document chunks). Several README claims not backed by code (social-media import, continuous learning, multi-agent collaboration, decentralized network). Standalone analysis `ANALYSIS-second-me.md` kept for triage-hygiene. 15.5k stars, Apache-2.0. |
| 2026-04-25 | OMEGA (omega-memory) | [omega-memory/core](https://github.com/omega-memory/core), PyPI `omega-memory` | **Not promoted — benchmark claim unverified/misattributed.** The `AgentMemoryWorld/Awesome-Agent-Memory` list flags OMEGA as "Ranks #1 on LongMemEval (95.4%)". **The actual OMEGA README makes no mention of LongMemEval or a 95.4% score** — the claim appears to be an awesome-list annotation not backed by the project itself. What OMEGA actually is: a local-first persistent-memory MCP server for AI coding agents with on-device ONNX embeddings + SQLite, Apache-2.0, 104 stars, 164 commits, 25 MCP memory tools. Small project; convergent with other local-first MCP memory layers (ByteRover CLI, Codex Memory). Re-examine only if a peer-reviewed LongMemEval number appears, or if the project claims the benchmark directly. |
| 2026-04-25 | StructMem (Xu et al., ACL 2026) | [arXiv:2604.21748](https://arxiv.org/abs/2604.21748), code [zjunlp/LightMem](https://github.com/zjunlp/LightMem) | **PROMOTED to paper deep-dive.** Hierarchical memory that emulates graph-like event coherence without building a KG: per-utterance dual-perspective extraction (factual + relational entries) stored as `⟨text, embedding, timestamp⟩`; event reconstruction at read time via timestamp grouping; periodic cross-event consolidation (K=15 seed retrieval → LLM synthesizes relational hypotheses → stored back as `C_cons`); dual-circuit QA retrieval (60 atomic + 5 synthesis). Flat vector store only (text-embedding-3-small); gpt-4o-mini for extraction/consolidation/QA. **LoCoMo overall 76.82** (beats Zep 75.14, Mem0 66.88, A-Mem 64.16; ties/beats Memobase 75.78 — best-in-table temporal 81.62). **~18× fewer build tokens than Mem0g** (1.937M vs 35.825M), ~50× fewer API calls, ~5× faster build. Ablation isolates cross-event consolidation as the source of multi-hop (+2.46pp) and temporal (+2.18pp) gains. **Explicit limitations (paper-flagged)**: no conflict resolution, no versioning, no decay, no confidence, LoCoMo-only evaluation, build-only efficiency numbers. Authors: Zhejiang University + Ant Group (ZJU–Ant Joint Lab of Knowledge Graph). Standalone analysis: `ANALYSIS-arxiv-2604.21748-structmem.md`; reference summary: `references/xu-structmem.md`. |
