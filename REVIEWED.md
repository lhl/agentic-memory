...
|------|--------|--------|---------|
| 2026-04-25 | Memobase (memodb-io) | [memodb-io/memobase](https://github.com/memodb-io/memobase) | **Candidate for promotion.** User-profile-based memory system (FastAPI + Postgres + Redis + Docker) emphasizing structured user profiles (name, interests, work, psychology, etc.) over raw conversational chunks. Targets three axes: retrieval performance, low LLM cost (batching buffer; claims ~40–50% token reduction vs per-event), sub-100ms latency via pre-built profiles + minimal SQL. **Real LoCoMo numbers surfaced in StructMem's Table 1 (overall 75.78, temporal 85.05 — best-in-table on temporal, beats Zep's 67.71)**; these are numbers we should engage with. 2.7k stars, 210 forks, 467 commits, Apache-2.0. SDKs: Python/Node/Go + MCP. Promote when we can get independent verification of the LoCoMo numbers or find a paper citation. |
| 2026-04-25 | Cognee (topoteretes) | [topoteretes/cognee](https://github.com/topoteretes/cognee) — arXiv:[2505.24478](https://arxiv.org/abs/2505.24478) | **Candidate for promotion.** Open-source knowledge engine combining vector search + graph DB (Neo4j support) + LLM extraction pipelines; API exposes four memory operations: `remember`/`recall`/`forget`/`improve` with auto-routing to select search strategy. Has a real paper ("Optimizing the Interface Between Knowledge Graphs and LLMs for Complex Reasoning", Markovic et al., 2025). 16.8k stars, 1.7k forks, 7k commits, Apache-2.0 — one of the most starred memory-as-a-service projects. Worth a deep-dive treatment similar to Supermemory/Zep; triage now, promote when we have capacity. |
| 2026-04-25 | Second Me (mindverse) | [mindverse/Second-Me](https://github.com/mindverse/Second-Me) — arXiv:[2503.08102](https://arxiv.org/abs/2503.08102) | **Candidate for promotion.** Open-source personal-AI-identity system with **Hierarchical Memory Modeling (HMM)** + **Me-Alignment Algorithm**; runs fully local via Docker (llama.cpp + Qwen2.5 base models, GPU/Apple-Silicon/CPU). Framed as "craft your own AI self" — closer to persona+identity capture than conversational memory. Associated paper ("AI-native Memory 2.0") arXiv:2503.08102. 15.5k stars, 1.2k forks, Apache-2.0. Different enough from the executive-assistant / coding-agent / memory-as-service categories that it would add a new design-space reference point. Triage now, consider promotion with a personality/identity-memory framing. |
