...
| [xu-structmem](references/xu-structmem.md) | Xu et al. (ACL 2026) | **StructMem**: graph-free structured memory — dual-perspective extraction (paired factual + relational entries) anchored to shared utterance timestamps; periodic cross-event consolidation (seed retrieval + LLM synthesis) writes relational hypotheses back as new entries feeding a dual-circuit QA read path; LoCoMo overall 76.82, best-in-table temporal 81.62, ~18× lower build tokens than Mem0g; no conflict resolution / versioning / decay (author-flagged). Code: `zjunlp/LightMem`. |
...
| [ANALYSIS-shisad](ANALYSIS-shisad.md) | `shisa-ai/shisad` v0.7.0 release content (2026-04-23) | Standalone deep dive: 5-surface architecture (shipped), formal trust model in code (PEP-minted handles + 14-rule matrix + SHA-256 content binding), identity candidate lifecycle, procedural install/invocation separation, sandboxed consolidation worker, adversarial eval scaffolding, cross-field comparison |
...
| [ANALYSIS-memobase](ANALYSIS-memobase.md) | `vendor/memobase/` | **Memobase (memodb-io)**: user-profile-based memory system (FastAPI + Postgres + Redis + pgvector). Typed 8-topic / ~40-slot ontology, cold-path batch architecture (buffer → fixed 3-call YOLO merge), dual profile/event channels. Reproducible LoCoMo artifacts shipped: overall 75.78, temporal 85.05 (best-in-table, beats Zep 67.71, Mem0 55.51). Apache-2.0, Python/TS/Go SDKs + MCP. Gaps: no provenance/versioning/trust labels, LoCoMo-only. |
| [ANALYSIS-cognee](ANALYSIS-cognee.md) | `vendor/cognee/` | **Cognee (topoteretes)**: knowledge-graph-backed memory engine (~138k LOC Python, Apache-2.0, 16.8k stars). Multi-backend (Kuzu/Neo4j/Neptune + 6 vector stores + SQLite/Postgres). Central novelty: bidirectional session-graph sync with feedback-weighted consolidation (`improve()`); 13+ search types with rule-based router; `@agent_memory` decorator; multi-tenant RBAC with shared datasets. Paper arXiv:2505.24478 covers KG-LLM interface optimization. Gaps: no LoCoMo (only HotpotQA), no supersedes/bi-temporal, hard-delete-only forgetting. |
| [ANALYSIS-second-me](ANALYSIS-second-me.md) | `vendor/second-me/` | **Second Me (mindverse)** — **not in agentic-memory scope** (kept as analyzed-but-excluded reference). Personal LLM fine-tuning pipeline: L0 (doc insights) → L1 (biography + shades) → L2 (synthetic QA + LoRA SFT + optional DPO) → GGUF → llama.cpp inference. "Memory" lives in LoRA adapter weights; update latency 30–60 min. Associated paper arXiv:2503.08102 "AI-native Memory 2.0". |
| [ANALYSIS-shisad](ANALYSIS-shisad.md) | `shisa-ai/shisad` v0.7.0 release content (2026-04-23) + design baseline `PLAN-longterm-memory.md` (private, 2026-04-21) | **shisad long-term memory** (v0.7.0 running-code analysis): most architecturally ambitious + security-conscious system in survey; 5-surface architecture (Identity/Active Attention/Recall/Procedural/Evidence) over unified SQLite typed-entry substrate (21 entry types / 6 categories); formal trust model in code (`memory/trust.py` 14-rule `_VALID_TRUST_MATRIX` + pending-review sentinel; `memory/ingress.py` PEP-minted `IngressContext` with SHA-256 content binding + `DerivationPath`); identity candidate lifecycle shipped; procedural memory with orthogonal install/invocation; 5-event confidence-update mechanics; sandboxed consolidation worker (explicit capability scope); strong-invalidation review flow; legacy backfill without offline migration; append-only `memory_events` trail; derived KG. Honest gaps: benchmark adapters (LoCoMo/LongMemEval/...) not yet in repo; adversarial track minimal (3-case poisoning fixture, utility_retention-only metric); `observed` trust band defined but gated off; Recall-surface MemoryPack annotation projection pending v0.7.1+ |
...
| [ANALYSIS-arxiv-2604.21748-structmem](ANALYSIS-arxiv-2604.21748-structmem.md) | `references/xu-structmem.md` | System deep dive on **StructMem** (ACL 2026): temporally-anchored dual-perspective extraction + periodic cross-event consolidation as a graph-free alternative to KG construction; head-to-head LoCoMo numbers vs Zep/Mem0/Memobase/A-Mem; ~18× build-token savings; mapping to shisad (dual-perspective extraction as `fact` + `relationship` types, synthesis entries with `consolidation_derived` trust band) |
...
├── ANALYSIS-memobase.md            ← Memobase: user-profile-ontology memory system (promoted)
├── ANALYSIS-cognee.md              ← Cognee: graph+session memory engine (promoted)
├── ANALYSIS-second-me.md           ← Second Me: personal LLM fine-tuning (not promoted)
├── ANALYSIS-shisad.md              ← shisad: v0.7.0 running-code analysis (released 2026-04-23)
